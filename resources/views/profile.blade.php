					...
					<div class="col-sm mb-4" style="width: 300px;">
						<form>
							...
							<script type="text/javascript">
								...
								async function text_ch(element)
								{
									var url = "/api/company/sears/?name_company=" + element.value
									let response = await fetch(url);
									if (response.ok) {
										let json = await response.json();
										var list = document.getElementById('clients-phone');
										if(json['error'] == "200")
										{
											json['return'].forEach(element => {
												var option = document.createElement('option');
											   	option.value = element['name_company'];
											   	option.title = element['id'];
											   	list.appendChild(option);
											});
										}
									}
									if (document.getElementById('id_clients-name').value != "" && document.getElementById('id_clients-name').value != "Пользователь не выбран") 
									{
										document.location.href = '/company/'+document.getElementById('id_clients-name').value
									}
